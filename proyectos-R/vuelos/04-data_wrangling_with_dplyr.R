...
##########################################################################
horas <- (foo$AirTime)/60
foo <- mutate(foo, avg_speed = Distance/horas)
...
foo <- filter(flights, DayOfWeek >=6 & DayOfWeek < 8, Dest == 'JFK', Distance > 1000, TaxiIn + TaxiOut < 15)

# 2) Combine the Year, Month and DayofMonth variables to create a Date column
foo <- mutate(foo, Date = as.Date(paste(Year, Month, DayofMonth, sep = "-")))

# 3) Result:

nrow(filter(foo, DayOfWeek %in% c(6,7), Distance > 1000, TaxiIn + TaxiOut < 15 ))
...
n_distinct((flights$UniqueCarrier))
...
summarise(
  ...
  avg_delay = mean(ArrDelay, na.rm = TRUE)  # na.rm borra los na para que pueda obtener media, si no los borro no podría hacerlo
)
...
set.seed(1973) 
(foo <- sample(1:10, 5, replace=T))  #generar numeros aleatorios
foo > 5
...
summarise(
  aa,
  n_security = sum(CancellationCode == "D", na.rm = TRUE) #Si no ponemos na.rm no hace bien la suma
)

sum(aa$CancellationCode == "B", na.rm = TRUE)

#PIPES:
# %>% OPERATOR ----------------------------------------------------------------------
# El símbolo %>% se escribe en linux con Ctrl + mayus + m
...
flights %>%
  ...
  filter(!is.na(diff)) %>% # aquí si queremos parar y ver unos resultado podemos agregar %>% tail()
  summarise(avg=mean(diff)) # si ponemos al final %>% View() nos abre ventana con el dataframe o resultado obtenido

# 
# flights %>%
#   mutate(diff=(TaxiOut-TaxiIn)) %>%
#   filter(!is.na(diff)) %>% tail(80) %>% readr::write_csv("data/foo2.csv")
...
# flights %>% 
#   filter(!is.na(DepTime) & !is.na(ArrTime)) %>% 
#     summarise(n2 =sum( ArrTime < DepTime))
...
# The result contains only a column named nplanes and a single row.

flights %>% 
  filter(Origin=="JFK") %>% 
   summarise(nplanes = sum(n_distinct(TailNum)))

# ahora los que han ido a un único destino:
...
flights %>% 
  group_by(Dest) %>% 
  summarise_at(vars(ends_with("Delay")), funs(mean), na.rm=T)
...
barplot(table(flights$UniqueCarrier))  #hace una tabla de frecuencia y luego la dibuja con barplot
...
flights %>% 
  ...
  mutate(DepTime = coalesce(DepTime, 0L))  #funcion coalesce fuerza a convertir una variable en el tipo que le pase 0L, esto es numérico entero

flights %>% 
  filter(is.na(DepTime)) %>% 
  mutate(DepTime = coalesce(DepTime, CRSDepTime)) #asigna a DepTime el valor que tenga en la columna CRSDepTime

unique(flights$CancellationCode)
foo <- flights %>% 
  mutate(CancellationCode = na_if(CancellationCode, "")) #transforma  un codigo de cancelación vacío a un NA
...
flights %>% 
  ...
  summarise(n = n()) %>% 
  arrange(-n) %>% 
  spread(Origin, n) %>%  #pivota en formato ancho
  gather("Origin", "n", 2:ncol(.)) %>% #vuelve a pivotar en formato largo
  arrange(-n) 
...
# Before joing dataframes, check for unique keys, comprobamos que no hay duplicado
airports %>% 
  count(iata) %>% # count nos da el número n
  filter(n > 1)
...
flights %>% 
  head %>%
  select(Year:DayofMonth,DepTime,ArrTime) %>%  #selecciona las columnas que queremos, desde Year hasta DayofMonth y DepTime, ArrTime
