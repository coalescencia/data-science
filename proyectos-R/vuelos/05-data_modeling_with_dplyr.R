...
flights <- query %>%  #ejecuta la consulta, guarda dataframe como resultado en flights
  collect() 

flights <- flights %>% sample_n(10000) 

model <- lm(DepDelay ~ ., data = flights)  #modelo estadístico de regresión lineal lm (linear model)
