...
dev.off() #These functions provide control over multiple graphics devices. Borra la pantalla de gráfico.
plot(clusterMovies)
...
movies$clusterGroups = cutree(clusterMovies, k = NumCluster) #con cutree cortamos por el número de cluster que digamos
...
tapply(movies$Action, movies$clusterGroups, mean) # por cada grupo nos da la media de a qué género pertenece
...
#normalizamos los datos con scale
...
# 
# set.seed(1234)
# Modelo=kmeans(creditosScaled[,1:5],NUM_CLUSTERS) #discriminamos sólo por 5 columnas
...
modelo11=lm(mpg~wt+qsec+am,data=coches)
summary(modelo11)

#hacemos la selección automática, que lo decida la aplicación utilizando step
modelo=step(modelo_bruto,trace=1,direction="both")
summary(modelo)
...
options(max.print = 99999) #para que muestre en pantalla organizado el summary
summary(samsungData)
## -------------------------------------------------------------------------
